...
class PDF(FPDF):
    ...
    def insertImage(self, x, design, textLevel):
        ...
        self.line(
            design + 5, textLevel + 5.55, design + 5, textLevel + 4.7 + ratio[key][1]
        )

        if ratio[key][0] < 50:
            self.line(
                design + 12.8,
                textLevel + 9.5 + ratio[key][1],
                design + ratio[key][0] + 12.5,
                textLevel + 9.5 + ratio[key][1],
            )
        else:
            self.line(
        ...
        if ratio[key][0] < 50:
            self.image(
                "./Images/{}.png".format(data["windowTypeVar"][x]),
                x=design + 12.5,
                y=textLevel + 5,
                w=ratio[key][0],
                h=ratio[key][1],
        ...
        self.cell(
            w=strWidth + 4, txt="{}".format(data["Width"][x]), align="C", fill=True
        )
    ...
    def printSpecs(self, textLevel, specs, x):
        ...
        for i in varName.keys():
            if data.iloc[x][i] != "" and data.iloc[x][i] != 0:
                self.multi_cell(
                    w=40, h=3.5, txt="{}: {}".format(varName[i], data.iloc[x][i])
                )
    ...
    def totalDisplay(self):
        ...
        indCurr = lambda x: format_currency(x, "INR", locale="en_IN").replace(
            "\xa0", " "
        )
        ...
        self.cell(
            w=40,
            h=6,
            txt="Rs. {}".format(data["totQuanSum"][0][1:]),
            fill=True,
            border=True,
            align="C",
        )
        ...
        self.set_fill_color(229, 233, 243)
        self.cell(
            w=20, h=6, txt="{} Nos".format(round(sum(data["quantity"]), 2)), align="L"
        )
        ...
        self.set_fill_color(229, 233, 243)
        self.cell(
            w=40, h=7, txt="Rs. {}".format(cgst[1:]), fill=True, border=True, align="C"
        )
        ...
        self.set_fill_color(229, 233, 243)
        self.cell(
            w=40, h=7, txt="Rs. {}".format(sgst[1:]), fill=True, border=True, align="C"
        )
        ...
        if data["installationFlag"][0] == "True Flag":
            ...
            self.set_fill_color(229, 233, 243)
            self.cell(
                w=45, h=7, txt="Installation Charges", fill=True, border=True, align="C"
            )
...
def open_file1():
    ...
    filename = askopenfile(
        title="Open a File", filetype=(("xlxs files", ".*xlsx"), ("All Files", "*."))
    )
...
def toExcel():
    ...
    if calculateFlag == True:
        ...
        messagebox.showerror(
            "Invalid",
            "Please fill in the cost field and click on the calculate button.",
            parent=new,
        )
        return
...
class PDFInvoice(FPDF):
    ...
    def infosection(self):
        ...
        self.set_font("helvetica", "B", 10)
        self.multi_cell(
            w=160, h=5, txt="Indian Rupees {} Only".format(billWords), align="L"
        )
...
class Invoice:
    ...
    def generateInvoicePDF(self):
        ...
        data.to_excel(
            "./Data/{}_QuatationData.xlsx".format(custNamVar.get()), index=False
        )
    ...
    def invWindow(self):
        ...
        custDetails = ttk.Label(
            self.new, text="Customer Details", font=("", 10, "bold")
        )
        custDetails.grid(column=0, row=0, padx=10, pady=10, sticky="W")

        frame2 = tk.LabelFrame(
            self.new, borderwidth=2, width=1000, labelwidget=custDetails
        )
        ...
        cartDetails = ttk.Label(
            frame_buttons, text="Cart Details", font=("", 10, "bold")
        )
        ...
        for i in range(data[data.columns[0]].count()):
            ...
            head3 = tk.Entry(
                frame_buttons,
                text=data["hsnSacVar"][x],
                textvariable=self.finAmt[i],
                relief="solid",
                width=13,
            )
        ...
        invoice = tk.Button(
            new, text="Generate Invoice PDF", width=20, command=self.generateInvoicePDF
        )
...
class CalculatePage:
    ...
    def generatePDF(self):
        ...
        if pressedCalcFlag != True:
            messagebox.showerror(
                "Invalid", "Please press calculate button", parent=self.new
            )
            pressedCalcFlag == False
            return False
    ...
    def calculateCost(self):
        ...
        if gstEntVar.get() == "":
            messagebox.showerror(
                "Invalid", "Please fill GST percentage", parent=self.new
            )
            return False
    ...
    def costPage(self):
        ...
        self.discountEnt = tk.Entry(
            new, textvariable=discountEntVar, relief="solid", width=16
        )
        ...
        self.costTot = tk.Entry(
            new, textvariable=costTotVar, relief="solid", width=16, state="disabled"
        )
        self.costTot.grid(column=3, row=2, padx=10, pady=10, sticky="W")

        self.discTot = tk.Entry(
            new, textvariable=discTotVar, relief="solid", width=16, state="disabled"
        )
        self.discTot.grid(column=3, row=4, padx=10, pady=10, sticky="W")
        self.instTot = tk.Entry(
            new, textvariable=instTotVar, relief="solid", width=16, state="disabled"
        )
        self.instTot.grid(column=3, row=6, padx=10, pady=10, sticky="W")
        self.gstTot = tk.Entry(
            new, textvariable=gstTotVar, relief="solid", width=16, state="disabled"
        )
        self.gstTot.grid(column=3, row=5, padx=10, pady=10, sticky="W")

        calculate = tk.Button(
            new, text="Calculate", width=20, command=self.calculateCost
        )
        ...
        invoice = tk.Button(
            new, text="Invoice Page", width=20, command=self.callInvoice
        )
...
class Cart:
    ...
    def nextItem(self):
        ...
        custDetails = ttk.Label(
            frame0, text="Enter Customer Details", font=("", 10, "bold")
        )
        custDetails.grid(column=0, row=0, padx=10, pady=10, sticky="W")
        ...
        custDetails = ttk.Label(
            frame1, text="Enter Window Details", font=("", 10, "bold")
        )
        custDetails.grid(column=0, row=0, padx=10, pady=10, sticky="W")
        ...
        selectWinDrop = ttk.Combobox(
            frame1, state="disabled", textvariable=windowTypeVar, width=23
        )
        ...
        nextButt = tk.Button(
            frame1, text="Next", font=("", 10, "bold"), width=19, command=selector
        )
    ...
    def addNewItem(self):
        ...
        selectWinLabel.grid(column=0, row=5, padx=10, pady=10, sticky="W")
        selectWinDrop = ttk.Combobox(
            frame1, state="readonly", textvariable=windowTypeVar, width=23
        )
        selectWinDrop["values"] = window_options
        selectWinDrop.grid(column=1, row=5, padx=10, pady=10, sticky="W")
        selectWinDrop.bind("<<ComboboxSelected>>", lambda e: selectWinLabel.focus())
        ...
        widthLabel.grid(column=0, row=6, padx=10, pady=10, sticky="W")
        enterWidth = tk.Entry(
            frame1, textvariable=Width, font=("", 10, ""), relief="solid", width=23
        )
        enterWidth.grid(column=1, row=6, padx=10, pady=10, sticky="W")
        heightLabel = ttk.Label(frame1, text="Enter Height (ft)")
        heightLabel.grid(column=0, row=7, padx=10, pady=10, sticky="W")
        enterHeight = tk.Entry(
            frame1, textvariable=Height, font=("", 10, ""), relief="solid", width=23
        )
        enterHeight.grid(column=1, row=7, padx=10, pady=10, sticky="W")
    ...
    def calcQuant(self):
        ...
        for i in range(data[data.columns[0]].count()):
            if data["quantity"][i] == "":
                messagebox.showerror(
                    "Invalid", "Please fill quantities of every item.", parent=self.new
                )
                return False
    ...
    def addMorePage(self):
        ...
        for i in range(data[data.columns[0]].count()):
            ...
            head5.grid(column=4, row=i + 2, padx=10, pady=10, sticky="W")
            quantity = tk.Entry(
                frame_buttons, textvariable=self.qt[i], relief="solid", width=12
            )
            ...
            delete = tk.Button(
                ...
                width=2,
                fg="red",
                command=lambda num=data.iloc[i]["Sr.No"]: self.deleteRow(num),
            )
...
class SlidingWindow:
    ...
    def calcCost(self):
        ...
        if costEntVar.get() == "":
            messagebox.showerror(
                "Invalid", "Please fill in the cost field.", parent=self.new
            )
            calculateFlag = False
            return False
        if not costEntVar.get().isdigit() and localFlag == False:
            messagebox.showerror(
                "Invalid", "Please enter numbers in the cost field.", parent=self.new
            )
            calculateFlag = False
            return False
        else:
    ...
    def frameCreate(self):
        ...
        selectWinLabel = ttk.Label(
            self.frame0, text=selectWinDrop.get(), font=("", 10, "bold")
        )
        ...
        enterWidth = tk.Entry(
            self.frame0, textvariable=Width, relief="solid", width=33, state="disabled"
        )
        ...
        enterHeight = tk.Entry(
            self.frame0, textvariable=Height, relief="solid", width=33, state="disabled"
        )
        ...
        self.costEnt = tk.Entry(
            self.frame0, textvariable=costEntVar, relief="solid", width=28
        )
        ...
        calculate = tk.Button(
            self.frame0, text="Calculate", width=34, command=self.calcCost
        )
        ...
        selectWinLabel = ttk.Label(
            self.frame1, text=selectWinDrop.get(), font=("", 20, "bold")
        )
    ...
    def variables(self):
        ...
        track = ttk.Combobox(
            self.frame1, textvariable=trackVar, width=30, values=track_options
        )
        track.grid(column=1, row=5, padx=10, pady=10, sticky="W")
        track.bind(
            "<Key>", lambda event: self.handleWait(event, track, "track_options")
        )

        aluMat = ttk.Combobox(
            self.frame1, textvariable=aluMatVar, width=30, values=aluminium_material
        )
        aluMat.grid(column=1, row=6, padx=10, pady=10, sticky="W")
        aluMat.bind(
            "<Key>", lambda event: self.handleWait(event, aluMat, "aluminium_material")
        )

        glaThic = ttk.Combobox(
            self.frame1, textvariable=glaThicVar, width=30, values=glass_thickness
        )
        glaThic.grid(column=1, row=7, padx=10, pady=10, sticky="W")
        glaThic.bind(
            "<Key>", lambda event: self.handleWait(event, glaThic, "glass_thickness")
        )

        glaTyp = ttk.Combobox(
            self.frame1, textvariable=glaTypVar, width=30, values=glass_type
        )
        ...
        hardLoc = ttk.Combobox(
            self.frame1, textvariable=hardLocVar, width=30, values=hardware_lock
        )
        hardLoc.grid(column=1, row=9, padx=10, pady=10, sticky="W")
        hardLoc.bind(
            "<Key>", lambda event: self.handleWait(event, hardLoc, "hardware_lock")
        )

        hardBea = ttk.Combobox(
            self.frame1, textvariable=hardBeaVar, width=30, values=hardware_bear
        )
        hardBea.grid(column=1, row=10, padx=10, pady=10, sticky="W")
        hardBea.bind(
            "<Key>", lambda event: self.handleWait(event, hardBea, "hardware_bear")
        )

        rubbTyp = ttk.Combobox(
            self.frame1, textvariable=rubbTypVar, width=30, values=rubber_type
        )
        rubbTyp.grid(column=1, row=11, padx=10, pady=10, sticky="W")
        rubbTyp.bind(
            "<Key>", lambda event: self.handleWait(event, rubbTyp, "rubber_type")
        )

        rubbThic = ttk.Combobox(
            self.frame1, textvariable=rubbThicVar, width=30, values=rubber_thick
        )
        rubbThic.grid(column=1, row=12, padx=10, pady=10, sticky="W")
        rubbThic.bind(
            "<Key>", lambda event: self.handleWait(event, rubbThic, "rubber_thick")
        )
        ...
        aluNet = ttk.Combobox(
            self.frame1, textvariable=aluNetVar, width=30, values=aluminium_net
        )
        aluNet.grid(column=1, row=14, padx=10, pady=10, sticky="W")
        aluNet.bind(
            "<Key>", lambda event: self.handleWait(event, aluNet, "aluminium_net")
        )

        fraCol = ttk.Combobox(
            self.frame1, textvariable=fraColVar, width=30, values=frame_colour
        )
        fraCol.grid(column=1, row=15, padx=10, pady=10, sticky="W")
        fraCol.bind(
            "<Key>", lambda event: self.handleWait(event, fraCol, "frame_colour")
        )

        silCol = ttk.Combobox(
            self.frame1, textvariable=silColVar, width=30, values=silicon_colour
        )
        silCol.grid(column=1, row=16, padx=10, pady=10, sticky="W")
        silCol.bind(
            "<Key>", lambda event: self.handleWait(event, silCol, "silicon_colour")
        )
        ...
        nextButt = tk.Button(
            self.frame1, text="Next", font=("", 10, "bold"), width=24, command=toExcel
        )
...
class FixLouver:
    ...
    def variables(self):
        ...
        lowBlad = ttk.Combobox(
            self.frame1, textvariable=lowBladVar, width=30, values=Louver_blade
        )
        lowBlad.grid(column=1, row=8, padx=10, pady=10, sticky="W")
        lowBlad.bind(
            "<Key>", lambda event: self.handleWait(event, lowBlad, "Louver_blade")
        )
...
class OpenableWindow:
    ...
    def variables(self):
        ...
        handle = ttk.Combobox(
            self.frame1, textvariable=handleVar, width=30, values=handle_options
        )
        handle.grid(column=1, row=17, padx=10, pady=10, sticky="W")
        handle.bind(
            "<Key>", lambda event: self.handleWait(event, handle, "handle_options")
        )
...
class SlidingFoldingDoor:
    ...
    def variables(self):
        ...
        handle = ttk.Combobox(
            self.frame1, textvariable=handleVar, width=30, values=channel_options
        )
        handle.grid(column=1, row=17, padx=10, pady=10, sticky="W")
        handle.bind(
            "<Key>", lambda event: self.handleWait(event, handle, "channel_options")
        )
...
class AluminiumPartition:
    ...
    def variables(self):
        ...
        acrShe = ttk.Combobox(
            self.frame1, textvariable=acrSheVar, width=30, values=acrylic_options
        )
        acrShe.grid(column=1, row=8, padx=10, pady=10, sticky="W")
        acrShe.bind(
            "<Key>", lambda event: self.handleWait(event, acrShe, "acrylic_options")
        )
...
class CompositePanel:
    ...
    def variables(self):
        ...
        compShe = ttk.Combobox(
            self.frame1, textvariable=compSheVar, width=30, values=composite_options
        )
        compShe.grid(column=1, row=6, padx=10, pady=10, sticky="W")
        compShe.bind(
            "<Key>", lambda event: self.handleWait(event, compShe, "composite_options")
        )
        ...
        maskTap = ttk.Combobox(
            self.frame1, textvariable=maskTapVar, width=30, values=masktape_options
        )
        maskTap.grid(column=1, row=9, padx=10, pady=10, sticky="W")
        maskTap.bind(
            "<Key>", lambda event: self.handleWait(event, maskTap, "masktape_options")
        )
...
class CurtainWall:
    ...
    def variables(self):
        ...
        acpShe = ttk.Combobox(
            self.frame1, textvariable=acpSheVar, width=30, values=acpsheet_options
        )
        acpShe.grid(column=1, row=7, padx=10, pady=10, sticky="W")
        acpShe.bind(
            "<Key>", lambda event: self.handleWait(event, glaTyp, "acpsheet_options")
        )
...
custNamEnt = tk.Entry(
    frame0, textvariable=custNamVar, font=("", 10, ""), relief="solid", width=21
)
custNamEnt.grid(column=1, row=1, padx=10, pady=10, sticky="W")
custAddEnt = tk.Text(frame0, font=("", 10, ""), relief="solid", height=3, width=21)
custAddEnt.grid(column=1, row=2, padx=10, pady=10, sticky="W")
custConEnt = tk.Entry(
    frame0, textvariable=custConVar, font=("", 10, ""), relief="solid", width=21
)
custConEnt.grid(column=1, row=3, padx=10, pady=10, sticky="W")
...
selectWinLabel.grid(column=0, row=5, padx=10, pady=10, sticky="W")
selectWinDrop = ttk.Combobox(
    frame1, state="readonly", textvariable=windowTypeVar, width=27
)
selectWinDrop["values"] = window_options
selectWinDrop.grid(column=1, row=5, padx=10, pady=10, sticky="W")
selectWinDrop.bind("<<ComboboxSelected>>", lambda e: selectWinLabel.focus())
...
widthLabel.grid(column=0, row=6, padx=10, pady=10, sticky="W")
enterWidth = tk.Entry(
    frame1, textvariable=Width, font=("", 10, ""), relief="solid", width=27
)
enterWidth.grid(column=1, row=6, padx=10, pady=10, sticky="W")
heightLabel = ttk.Label(frame1, text="Enter Height (ft)")
heightLabel.grid(column=0, row=7, padx=10, pady=10, sticky="W")
enterHeight = tk.Entry(
    frame1, textvariable=Height, font=("", 10, ""), relief="solid", width=27
)
enterHeight.grid(column=1, row=7, padx=10, pady=10, sticky="W")
